...
namespace ULib.Winform
{
	...
	{
		private const string STR_Embeded = "Embeded";
		private const string STR_Covered = "Covered";
		...
		private static void ClearControls(Control parent)
		{
			foreach (Control item in parent.Controls)
			{
				try
				{
					Form itemForm = item as Form;
					MethodInfo mi = item.GetType().GetMethod(STR_Covered);
					if (mi != null)
					{
						mi.Invoke(item, null);
					}
