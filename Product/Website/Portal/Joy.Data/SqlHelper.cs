...
namespace Joy.Data
{
	...
	public class SqlObject
	{
		...
		public SqlObject InsertInto(string table, DataTable tb)
		{
			if (tb == null)
			{
				return this;
			...
			foreach (DataColumn i in tb.Columns)
			{
				list.Add(i.ColumnName);
			}
			list.RemoveAt(0);
			return InsertInto(table, list.ToArray());
		}
		...
		public SqlObject Values(DataRow row)
		{
			if (row == null)
			{
				return this;
			...
			for (int i = 0; i < row.Table.Columns.Count; i++)
			{
				...
				if (ri != null)
				{
					...
					string val = SqlHelper.MakeSafeFieldValue(ri.ToString(), col.DataType.Name, true);
					list.Add(val);
				}
				else
				{
			...
			list.RemoveAt(0);
			return Values(string.Join(",", list.ToArray()));
		}
