...
{
	public class OwinAdapter : IDisposable
	{
		protected int port { get; }
		protected dynamic settings { get; }
		protected StartOptions options { get; }
		protected ManualResetEvent mre { get; }
		protected bool isdisposed { get; set; }

		public OwinAdapter(Dobj config)
		{
			settings = config;
			string cn = Dobj.Exists(settings, "cn") ? settings.cn : "localhost";
			string d = Dobj.Exists(settings, "domain")?settings.domain:"+";
			bool ssl = Dobj.Get<bool>(settings, "ssl");
			var port = settings.port;
			mre = new ManualResetEvent(false);
			this.port = port;
			if (ssl)
			{
				var file = AppDomain.CurrentDomain.BaseDirectory + "cert.pfx";
				var cert = GetCert(cn, TimeSpan.FromDays(3650), "addincer", file);
				ActivateCert((X509Certificate2) cert, port, GetAppId());
			}
			var so = new StartOptions();
			so.Urls.Add($"{(ssl ? "https" : "http")}://{d}:{port}/");
			options = so;
		}

		public void Start(params object[] args)
		{
			log.i(AppDomain.CurrentDomain.BaseDirectory);
			Thread th = new Thread(new ThreadStart(() =>
			{
				using (WebApp.Start<OwinStartup>(options))
				{
	...
	public class OwinStartup
	{
		...
		public void Configuration(IAppBuilder app)
		{
			...
			//});
			app.Use(typeof(ExecuteMiddleware));
	...
	/// Add Microsoft.Owin.Host.HttpListener.dll into addin folder
	/// </summary>
	public class NoUseServerFactory
	{
		private OwinHttpListener listener;
	}
}