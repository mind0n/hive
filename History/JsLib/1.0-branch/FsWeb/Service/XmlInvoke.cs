...
namespace FsWeb.Service
{
	...
	public class RequestXmlInvoker
	{
		...
		public void Invoke(object vtarget, HttpRequest request, HttpResponse response)
		{
			...
			response.Write("<?xml version=\"1.0\" encoding=\"utf-8\" ?><result successful=\"true\">");
			for (int i = 0; i < Results.Count; i++)
			{
				InvokeResult rlt = (InvokeResult)Results[i];
				if (rlt != null)
				{
					response.Write("\n<method name=\"" + rlt.MethodName + "\" successful=\"" + (!rlt.HasError).ToString().ToLower() + "\"><content>" + rlt.ReturnValue<string>() + "</content>");
					if (rlt.HasError)
					{
