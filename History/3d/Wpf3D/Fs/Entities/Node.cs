...
namespace Fs.Entities
{
	...
	public class Node : Dictionary<string, Node>
	{
		...
		protected int _id;
		virtual public void SetValue(object val, string typ, string key)
		{
		...
		virtual public void SetValue(object val, NodeType typ, string key)
		{
			if (string.IsNullOrEmpty(key))
			{
				Value = val;
		...
		virtual public bool AddChild(object val, NodeType typ)
		{
			string key;
			_id++;
			key = "_" + _id;
		...
		virtual public bool AddChild(string key, NodeType typ)
		{
			return AddChild(null, typ, key);
		}
		virtual public bool AddChild(object val, NodeType typ, string key)
		{
			if (!this.ContainsKey(key))
			{
				SetValue(val, typ, key);
